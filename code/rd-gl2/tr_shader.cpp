	...
	qboolean useLightVector, qboolean useLightVertex, qboolean tcgen)
{
	...
	if (r_specularMapping->integer)
	{
		...
		if (stage->bundle[TB_SPECULARMAP].image[0])
		{
			//R_Printf(PRINT_ALL, ", specularmap %s", stage->bundle[TB_SPECULARMAP].image[0]->imgName);
		}
		else if ((lightmap || useLightVector || useLightVertex) && (diffuseImg = stage->bundle[TB_DIFFUSEMAP].image[0]))
		{
...
static void ScanAndLoadShaderFiles(void)
{
	...
	{
		R_Error(ERR_FATAL, "WARNING: no shader files found\n");
		return;
	}
	...
	for (i = 0; i < numShaderFiles; i++)
	{
		...
		// look for a .mtr file first
		{
			...
			{
				R_Printf(PRINT_ALL, "...loading MTR shader '%s'\n", filename);
			}
		}
...
static void CreateExternalShaders( void ) {
	...
	{
		R_Printf(PRINT_ALL, "Sun flare shader is missing. Generating from flare or dlight image\n");
