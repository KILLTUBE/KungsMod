...
void LoadJPG( const char *filename, unsigned char **pic, int *width, int *height ) {
	...
	*/
		...
		)
	{
		...
		R_Printf( PRINT_ALL, "LoadJPG: %s has an invalid image format: %dx%d*4=%d, components: %d", filename,
...
void LoadJPGFromBuffer( byte *inputBuffer, size_t len, unsigned char **pic, int *width, int *height ) {
	   ...
	   )
	{
		...
		R_Printf( PRINT_ALL, "LoadJPG: invalid image format: %dx%d*4=%d, components: %d",
				  cinfo.output_width, cinfo.output_height, pixelcount * 4, cinfo.output_components);
		return;
	}
