...
//				R_Printf( PRINT_DEVELOPER, "Dumping \"%s\"", psModelName);
//				R_Printf( PRINT_DEVELOPER, ", used on lvl %d\n",CachedModel.iLastLevelUsedOn);
	#endif
	...
	//R_Printf( PRINT_DEVELOPER, "RE_RegisterModels_LevelLoadEnd(): Ok\n");
...
*/
	...
	if ( !name || !name[0] ) {
		R_Printf( PRINT_WARNING, "RE_RegisterModel: NULL name\n" );
		return 0;
	}

	if ( strlen( name ) >= MAX_QPATH ) {
		R_Printf( PRINT_DEVELOPER, "Model name exceeds MAX_QPATH\n" );
		return 0;
	}
...
*/
//	if (!tr.registered) {
//		R_Printf( PRINT_WARNING, "RE_RegisterModel (%s) called before ready!\n",name );
//		return 0;
//	}
	//
	...
	for ( lod = iLODStart; lod >= 0 ; lod-- ) {
		...
		switch (ident)
		{
			...
			default:
				...
				R_Printf (PRINT_WARNING,"RE_RegisterModel: unknown fileid for %s\n", filename);
				goto fail;
		}
		...
		if ( !loaded ) {
			if ( lod == 0 ) {
				R_Printf (PRINT_WARNING,"RE_RegisterModel: cannot load %s\n", filename);
				goto fail;
			} else {
				break;
			}
...
void R_Modellist_f( void ) {
	...
	for ( i = 1 ; i < tr.numModels; i++ ) {
		...
		switch (mod->type)
		{
			default:
				assert(0);
				R_Printf( PRINT_ALL, "UNKNOWN  :      %s\n", mod->name );
				break;

			case MOD_BAD:
				R_Printf( PRINT_ALL, "MOD_BAD  :      %s\n", mod->name );
				break;
