...
//		totalFileSizeK += (image->imgfileSize+1023)/1024;
		//R_Printf (PRINT_ALL,  "%4i: %4i %4i %5i  %s ",
...
//	R_Printf (PRINT_ALL, " %iMB total filesize\n", (totalFileSizeK+1023)/1024 );
...
void RE_RegisterImages_Info_f( void )
{
	...
	while ( (pImage	= R_Images_GetNextIteration()) != NULL)
	{
		...
		R_Printf( PRINT_ALL, ", levused %d",pImage->iLastLevelUsedOn);
		R_Printf( PRINT_ALL, "\n");
	...
	R_Printf( PRINT_DEVELOPER, "RE_RegisterMedia_GetLevel(): %d",RE_RegisterMedia_GetLevel());
}
...
{
	//R_Printf( PRINT_DEVELOPER, "RE_RegisterImages_LevelLoadEnd():\n");
	...
	for (AllocatedImages_t::iterator itImage = AllocatedImages.begin(); itImage != AllocatedImages.end(); /* blank */)
	{
		...
		if (pImage->imgName[0] != '*')
		{
			...
			{	// nope, so dump it...
				//R_Printf( PRINT_DEVELOPER, "Dumping image \"%s\"\n",pImage->imgName);
				R_Images_DeleteImageContents(pImage);
	...
	//R_Printf( PRINT_DEVELOPER, "RE_RegisterImages_LevelLoadEnd(): Ok\n");
	...
	if (itAllocatedImage != AllocatedImages.end())
	{
		...
		if ( strcmp( pName, "*white" ) ) {
			if ( pImage->mipmap != !!mipmap ) {
				R_Printf( PRINT_WARNING, "WARNING: reused image %s with mixed mipmap parm\n", pName );
			}
			if ( pImage->allowPicmip != !!allowPicmip ) {
				R_Printf( PRINT_WARNING, "WARNING: reused image %s with mixed allowPicmip parm\n", pName );
			}
			if ( pImage->wrapClampMode != glWrapClampMode ) {
				R_Printf( PRINT_WARNING, "WARNING: reused image %s with mixed glWrapClampMode parm\n", pName );
			}
		}
