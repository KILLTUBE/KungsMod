...
qhandle_t RE_RegisterIndividualSkin( const char *name , qhandle_t hSkin)
{
	...
	if ( !text ) {
		R_Printf( PRINT_WARNING, "WARNING: RE_RegisterSkin( '%s' ) failed to load!\n", name );
		return 0;
	}
	...
	while ( text_p && *text_p ) {
		...
		if ( (unsigned)skin->numSurfaces >= ARRAY_LEN( skin->surfaces ) )
		{
			...
			R_Printf( PRINT_WARNING, "WARNING: RE_RegisterSkin( '%s' ) more than %u surfaces!\n", name, (unsigned int)ARRAY_LEN(skin->surfaces) );
			break;
		}
...
//	}
	...
	if ( tr.numSkins == MAX_SKINS )	{
		R_Printf( PRINT_WARNING, "WARNING: RE_RegisterSkin( '%s' ) MAX_SKINS hit\n", name );
		return 0;
	}
	// allocate a new skin
	tr.numSkins++;
