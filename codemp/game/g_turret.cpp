...
void turret_head_think( gentity_t *self ) {
	...
	if ( self->enemy && self->setTime < level.time && self->attackDebounceTime < level.time )
	{
		...
		SV_G2API_GetBoltMatrix( self->ghoul2, self->playerModel,
					self->torsoBolt,
		...
		SV_G2API_GiveMeVectorFromMatrix( boltMatrix, POSITIVE_Y, fwd );
		*/
...
static void turret_aim( gentity_t *self ) {
	...
	else if ( self->enemy )
	{
		...
		SV_G2API_GiveMeVectorFromMatrix( boltMatrix, ORIGIN, org2 );
		*/
...
void SP_misc_turret( gentity_t *base ) {
	...
	//base->playerModel = SV_G2API_InitGhoul2Model( base->ghoul2, "models/map_objects/imp_mine/turret_canon.glm", base->s.modelindex );
	//base->s.radius = 80.0f;

	//SV_G2API_SetBoneAngles( &base->ghoul2[base->playerModel], "Bone_body", vec3_origin, BONE_ANGLES_POSTMULT, POSITIVE_Y, POSITIVE_Z, POSITIVE_X, NULL );
	//base->torsoBolt = SV_G2API_AddBolt( &base->ghoul2[base->playerModel], "*flash03" );
...
qboolean turret_base_spawn_top( gentity_t *base ) {
	...
	SV_LinkEntity( (sharedEntity_t *)top );
	return qtrue;
}
