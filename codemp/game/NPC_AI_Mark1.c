...
void Mark1Dead_FireRocket (void)
{
	...
	gentity_t *missile;
	int	damage	= 50;
	int bolt = SV_G2API_AddBolt(NPCS.NPC->ghoul2, 0, "*flash5");
...
void Mark1_dying( gentity_t *self )
{
	...
	if (self->client->ps.torsoTimer>0)
	{
		if (TIMER_Done(self,"dyingExplosion"))
		{
			...
			{
				num = Q_irand( 8, 10);
				newBolt = SV_G2API_AddBolt( self->ghoul2, 0, va("*flash%d",num) );
				NPC_Mark1_Part_Explode(self,newBolt);
			}
			else
			{
				num = Q_irand( 1, 6);
				newBolt = SV_G2API_AddBolt( self->ghoul2, 0, va("*torso_tube%d",num) );
				NPC_Mark1_Part_Explode(self,newBolt);
...
//		VectorMA( self->client->ps.velocity, -80, right, self->client->ps.velocity );
		...
		if (!SV_G2API_GetSurfaceRenderStatus( self->ghoul2, 0, "l_arm" ))	// Is the blaster still on the model?
		{
			if (Q_irand( 1, 5) == 1)
			{
				SaveNPCGlobals();
		...
		if (!SV_G2API_GetSurfaceRenderStatus( self->ghoul2, 0, "r_arm" ))	// Is the rocket still on the model?
		{
			if (Q_irand( 1, 10) == 1)
			{
				SaveNPCGlobals();
...
//				G_PlayEffect( "small_chunks", self->playerModel, self->genericBolt2, self->s.number);
	...
	else
	{
		for (i=0;i<6;i++)
		{
			if ((hitLoc==HL_GENERIC1+i) && (self->locationDamage[HL_GENERIC1+i] > AMMO_POD_HEALTH))	// Blow it up?
			{
				...
				{
					newBolt = SV_G2API_AddBolt( self->ghoul2, 0, va("*torso_tube%d",(i+1)) );
					if ( newBolt != -1 )
					{
						NPC_Mark1_Part_Explode(self,newBolt);
					}
	...
	if ((SV_G2API_GetSurfaceRenderStatus( self->ghoul2, 0, "l_arm" )>0) &&
		(SV_G2API_GetSurfaceRenderStatus( self->ghoul2, 0, "r_arm" )>0))
	{
...
//	static	vec3_t	muzzle;
	...
	else if (NPCS.NPCInfo->localState == LSTATE_FIRED2)
	{
		...
		bolt = SV_G2API_AddBolt(NPCS.NPC->ghoul2, 0, "*flash3");
	}
	else
	{
		NPCS.NPCInfo->localState = LSTATE_FIRED4;
		bolt = SV_G2API_AddBolt(NPCS.NPC->ghoul2, 0, "*flash4");
	}
...
//			G_Sound( NPC, G_SoundIndex(va("sound/chars/mark1/misc/talk%d.wav",	Q_irand(1, 4))));
	...
	blasterTest = SV_G2API_GetSurfaceRenderStatus( NPCS.NPC->ghoul2, 0, "l_arm" );
	rocketTest = SV_G2API_GetSurfaceRenderStatus( NPCS.NPC->ghoul2, 0, "r_arm" );
