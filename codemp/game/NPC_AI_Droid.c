...
void R2D2_PartsMove(void)
{
	...
	if ( TIMER_Done(NPCS.NPC,"eyeDelay") )
	{
		...
		if (NPC->genericBone1)
		{
			SV_G2API_SetBoneAnglesIndex( &NPC->ghoul2[NPC->playerModel], NPC->genericBone1, NPC->pos1, BONE_ANGLES_POSTMULT, POSITIVE_X, NEGATIVE_Y, NEGATIVE_Z, NULL );
		}
		*/
...
void Droid_Spin( void )
{
	...
	{
		// No head?
		if (SV_G2API_GetSurfaceRenderStatus( NPCS.NPC->ghoul2, 0, "head" )>0)
		{
				...
				&& ( self->health < 30 || mod == MOD_DEMP2 || mod == MOD_DEMP2_ALT ) )
			{
				if (!(self->spawnflags & 2))	// Doesn't have to ALWAYSDIE
				{
					if ((self->NPC->localState != LSTATE_SPINNING) &&
						(!SV_G2API_GetSurfaceRenderStatus( self->ghoul2, 0, "head" )))
					{
