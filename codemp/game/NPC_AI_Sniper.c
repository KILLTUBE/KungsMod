			...
			{//miss a few times first
				...
				{//ready to fire again
					...
					while ( hit && tryMissCount < 10 )
					{
						...
						SV_Trace( &trace, muzzle, vec3_origin, vec3_origin, target, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
...
void NPC_BSSniper_Attack( void )
{
	...
	{//too close, so switch to primary fire
		...
		{//sniping... should be assumed
			...
			{//use primary fire
				trace_t	trace;
				SV_Trace ( &trace, NPCS.NPC->enemy->r.currentOrigin, NPCS.NPC->enemy->r.mins, NPCS.NPC->enemy->r.maxs, NPCS.NPC->r.currentOrigin, NPCS.NPC->enemy->s.number, NPCS.NPC->enemy->clipmask, qfalse, 0, 0 );
	...
	if ( NPC_ClearLOS4( NPCS.NPC->enemy ) )//|| (NPCInfo->stats.aim >= 5 && SV_inPVS( NPC->client->renderInfo.eyePoint, NPC->enemy->currentOrigin )) )
	{
		float maxShootDist;
		...
		if ( enemyDist2 < maxShootDist )
		{
			...
			SV_Trace ( &tr, muzzle, NULL, NULL, end, NPCS.NPC->s.number, MASK_SHOT, qfalse, 0, 0 );
	...
	/*
	else if ( SV_inPVS( NPC->enemy->currentOrigin, NPC->currentOrigin ) )
	{
		NPCInfo->enemyLastSeenTime = level.time;
		faceEnemy2 = qtrue;
	}
