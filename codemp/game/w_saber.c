	...
	{ //sort of hacky, but it saves a pretty big load off the server
		...
		while (i < MAX_CLIENTS)
		{
			...
			if (clEnt && clEnt->inuse && clEnt->client &&
				SV_inPVS(clEnt->client->ps.origin, ent->client->ps.origin))
...
static QINLINE qboolean WP_SabersCheckLock2( gentity_t *attacker, gentity_t *defender, sabersLockMode_t lockMode )
{
	...
	SV_Trace( &trace, attacker->r.currentOrigin, attacker->r.mins, attacker->r.maxs, newOrg, attacker->s.number, attacker->clipmask, qfalse, 0, 0 );
	if ( !trace.startsolid && !trace.allsolid )
	{
		...
		SV_LinkEntity( (sharedEntity_t *)attacker );
	}
	...
	SV_Trace( &trace, defender->r.currentOrigin, defender->r.mins, defender->r.maxs, newOrg, defender->s.number, defender->clipmask, qfalse, 0, 0 );
	if ( !trace.startsolid && !trace.allsolid )
	{
		if (defender->client)
		{
		...
		SV_LinkEntity( (sharedEntity_t *)defender );
	}
...
static QINLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	...
	while (!saberTraceDone)
	{
		...
		{ //This didn't quite work out like I hoped. But it's better than nothing. Sort of.
			...
			SV_Trace(&tr, saberEnd, saberTrMins, saberTrMaxs, saberStart, self->s.number, trMask, qfalse, 0, 0);
		...
		else
		{
			...
			SV_Trace(&tr, saberStart, saberTrMins, saberTrMaxs, saberEndExtrapolated, self->s.number, trMask, qfalse, 0, 0);
	...
	for ( e = 0 ; e < numListedEntities ; e++ )
	{
		...
		{ //seems like a valid enemy to look at.
			...
			if (lookTDist == -1 || vecLen < lookTDist)
			{
				...
				SV_Trace(&tr, myEyes, NULL, NULL, ent->client->ps.origin, self->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
		...
		if ( dist < closestDist )
		{
			...
			SV_Trace( &trace, ent->r.currentOrigin, ent->r.mins, ent->r.maxs, traceTo, ent->s.number, ent->clipmask, qfalse, 0, 0 );
...
static QINLINE qboolean CheckThrownSaberDamaged(gentity_t *saberent, gentity_t *saberOwner, gentity_t *ent, int dist, int returning, qboolean noDCheck)
{
	...
	if (ent && ent->client && ent->inuse && ent->s.number != saberOwner->s.number &&
		...
		SV_inPVS(ent->client->ps.origin, saberent->r.currentOrigin) &&
		ent->client->sess.sessionTeam != TEAM_SPECTATOR &&
	...
	{ //hit a client
		...
		{ //within range
			...
			SV_Trace(&tr, saberent->r.currentOrigin, NULL, NULL, ent->client->ps.origin, saberent->s.number, MASK_SHOT, qfalse, 0, 0);
		...
		ent->s.number != saberent->s.number && (noDCheck ||SV_inPVS(ent->r.currentOrigin, saberent->r.currentOrigin)))
	{ //hit a non-client
		...
		if (veclen < dist)
		{
			...
			SV_Trace(&tr, saberent->r.currentOrigin, NULL, NULL, entOrigin, saberent->s.number, MASK_SHOT, qfalse, 0, 0);
...
static QINLINE void saberMoveBack( gentity_t *ent, qboolean goingBack )
{
	...
	{ //acts as a fallback in case touch code fails, keeps saber from going through things between predictions
		...
		SV_Trace(&tr, oldOrg, mins, maxs, compensatedOrigin, ent->r.ownerNum, MASK_PLAYERSOLID, qfalse, 0, 0);
...
{	//spawn a "dead" saber entity here so it looks like the saber fell out of the air.
	...
	SV_Trace(&tr, startorg, saberent->r.mins, saberent->r.maxs, startorg, saberent->s.number, saberent->clipmask, qfalse, 0, 0);
	...
	SV_LinkEntity((sharedEntity_t *)saberent);
}
...
void saberKnockDown(gentity_t *saberent, gentity_t *saberOwner, gentity_t *other)
{
	...
	//(normally at the player's right hand) could result in the saber being stuck
	//in the the map and then freaking out.
	SV_Trace(&tr, saberent->r.currentOrigin, saberent->r.mins, saberent->r.maxs, saberent->r.currentOrigin, saberent->s.number, saberent->clipmask, qfalse, 0, 0);
	if(tr.startsolid || tr.fraction != 1)
	{//bad position, try popping our origin up a bit
...
void saberFirstThrown(gentity_t *saberent)
{
	...
	{ //if owner is rank 3 in saber throwing, the saber goes where he points
		...
		{ //if highest saber throw rank, we can direct the saber toward players directly by looking at them
			SV_Trace(&tr, traceFrom, NULL, NULL, traceTo, saberOwn->s.number, MASK_PLAYERSOLID, qfalse, 0, 0);
		}
		else
		{
			SV_Trace(&tr, traceFrom, NULL, NULL, traceTo, saberOwn->s.number, MASK_SOLID, qfalse, 0, 0);
		}
...
static gentity_t *G_KickTrace( gentity_t *ent, vec3_t kickDir, float kickDist, vec3_t kickEnd, int kickDamage, float kickPush )
{
	...
	if (d_saberKickTweak.integer)
	{
		SV_Trace( &trace, traceOrg, kickMins, kickMaxs, traceEnd, ent->s.number, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
	}
	else
	{
		SV_Trace( &trace, traceOrg, kickMins, kickMaxs, traceEnd, ent->s.number, MASK_SHOT, qfalse, 0, 0 );
	}
...
static void G_GrabSomeMofos(gentity_t *self)
{
	...
	SV_Trace( &trace, self->client->ps.origin, grabMins, grabMaxs, pos, self->s.number, MASK_SHOT, qfalse, G2TRFLAG_DOGHOULTRACE|G2TRFLAG_GETSURFINDEX|G2TRFLAG_THICK|G2TRFLAG_HITCORPSES, g_g2TraceLod.integer );
		...
		else
		{
			...
			else
			{
				...
				{ //grapplee
					...
					SV_Trace(&trace, self->client->ps.origin, self->r.mins, self->r.maxs, idealSpot, self->s.number, self->clipmask, qfalse, 0, 0);
...
*/
	...
	*/
	...
	if (!BG_SabersOff(&self->client->ps))
	{
		...
		{
			SV_LinkEntity((sharedEntity_t *)mySaber);
		}
