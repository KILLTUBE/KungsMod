...
void NPC_RemoveBody( gentity_t *self )
{
	...
	{
		ICARUS_MaintainTaskManager(self->s.number);
	}
		...
		|| self->client->NPC_class == CLASS_MARK2 )
	{
		...
		if (!ICARUS_IsRunning(self->s.number))
		{
...
void pitch_roll_for_slope( gentity_t *forwhom, vec3_t pass_slope )
{
	...
	if( !pass_slope || VectorCompare( vec3_origin, pass_slope ) )
	{
		...
		endspot[2] -= 300;
		SV_Trace( &trace, forwhom->r.currentOrigin, vec3_origin, vec3_origin, endspot, forwhom->s.number, MASK_SOLID, qfalse, 0, 0 );
...
//			forwhom.flags(-)FL_ONGROUND;
	...
	if ( forwhom->client )
	{
		...
		{//our mins is now lower, need to move up
			...
			SV_LinkEntity( (sharedEntity_t *)forwhom );
		}
	}
	else
	{
...
static void DeadThink ( void )
{
	...
	{//not flying through the air
		...
		{
			NPCS.NPC->r.mins[0] -= 1;
			SV_Trace (&trace, NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, NPCS.NPC->r.currentOrigin, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0 );
			if ( trace.allsolid )
			{
				NPCS.NPC->r.mins[0] += 1;
			}
	...
	*/
	{
		...
		if ( level.time >= NPCS.NPCInfo->timeOfDeath + BodyRemovalPadTime( NPCS.NPC ) )
		{
			...
			{
				if (!ICARUS_IsRunning(NPCS.NPC->s.number))
	...
	if ( NPCS.NPC->bounceCount < 0 && NPCS.NPC->s.groundEntityNum >= 0 )
	{
		...
		int contents = NPCS.NPC->bounceCount = SV_PointContents( NPCS.NPC->r.currentOrigin, -1 );
...
void NPC_ShowDebugInfo (void)
{
	if ( showBBoxes )
	{
		...
		{
			if ( SV_inPVS( found->r.currentOrigin, g_entities[0].r.currentOrigin ) )
			{
...
//	else if ( NPCS.NPC->client->ps.weapon == WP_SABER )		// this is an _extremely_ shitty comparison.. FIXME: make a CLASS_CULTIST? --eez
	...
	//	case NPCTEAM_MALON:
		...
		case NPCTEAM_ENEMY:
			...
			if ( NPCS.NPC->enemy && NPCS.NPC->s.weapon == WP_NONE && bState != BS_HUNT_AND_KILL && !ICARUS_TaskIDPending( (sharedEntity_t *)NPCS.NPC, TID_MOVE_NAV ) )
...
void NPC_CheckInSolid(void)
{
	...
	SV_Trace(&trace, NPCS.NPC->r.currentOrigin, NPCS.NPC->r.mins, NPCS.NPC->r.maxs, point, NPCS.NPC->s.number, NPCS.NPC->clipmask, qfalse, 0, 0);
...
{
	/*
	Cvar_Register(&g_saberRealisticCombat, "g_saberRealisticCombat", "0", CVAR_CHEAT);

	Cvar_Register(&debugNoRoam, "d_noroam", "0", CVAR_CHEAT);
	Cvar_Register(&debugNPCAimingBeam, "d_npcaiming", "0", CVAR_CHEAT);
	Cvar_Register(&debugBreak, "d_break", "0", CVAR_CHEAT);
	Cvar_Register(&d_npcai, "d_npcai", "0", CVAR_CHEAT);
	Cvar_Register(&debugNPCFreeze, "d_npcfreeze", "0", CVAR_CHEAT);
	Cvar_Register(&d_JediAI, "d_JediAI", "0", CVAR_CHEAT);
	Cvar_Register(&d_noGroupAI, "d_noGroupAI", "0", CVAR_CHEAT);
	Cvar_Register(&d_asynchronousGroupAI, "d_asynchronousGroupAI", "0", CVAR_CHEAT);
	...
	Cvar_Register(&d_slowmodeath, "d_slowmodeath", "0", CVAR_CHEAT);

	Cvar_Register(&d_saberCombat, "d_saberCombat", "0", CVAR_CHEAT);

	Cvar_Register(&g_npcspskill, "g_npcspskill", "0", CVAR_ARCHIVE | CVAR_USERINFO);
	*/
}
...
{
//	globals.NPCs = (gNPC_t *) SV_TagMalloc(game.maxclients * sizeof(game.bots[0]), TAG_GAME);
//	Cvar_Register(&debugNPCName, "d_npc", "0", CVAR_CHEAT);
