...
qboolean G_ClearLineOfSight(const vec3_t point1, const vec3_t point2, int ignore, int clipmask)
{
	...
	SV_Trace ( &tr, point1, NULL, NULL, point2, ignore, clipmask, qfalse, 0, 0 );
	if ( tr.fraction == 1.0 )
	{
		return qtrue;
	}
	...
	if(EntIsGlass(hit))
	{
		...
		SV_Trace (&tr, newpoint1, NULL, NULL, point2, hit->s.number, clipmask, qfalse, 0, 0 );
...
qboolean CanSee ( gentity_t *ent )
{
	...
	SV_Trace ( &tr, eyes, NULL, NULL, spot, NPCS.NPC->s.number, MASK_OPAQUE, qfalse, 0, 0 );
...
visibility_t NPC_CheckVisibility ( gentity_t *ent, int flags )
{
	...
	if ( flags & CHECK_PVS )
	{
		if ( !SV_inPVS ( ent->r.currentOrigin, NPCS.NPC->r.currentOrigin ) )
		{
			return VIS_NOT;
		}
	}
...
qboolean G_ClearLOS( gentity_t *self, const vec3_t start, const vec3_t end )
{
	...
	SV_Trace ( &tr, start, NULL, NULL, end, ENTITYNUM_NONE, CONTENTS_OPAQUE/*CONTENTS_SOLID*//*(CONTENTS_SOLID|CONTENTS_MONSTERCLIP)*/, qfalse, 0, 0 );
	...
	{//can see through 3 panes of glass
		if ( tr.entityNum < ENTITYNUM_WORLD )
		{
			...
			{//can see through glass, trace again, ignoring me
				SV_Trace ( &tr, tr.endpos, NULL, NULL, end, tr.entityNum, MASK_OPAQUE, qfalse, 0, 0 );
				traceCount++;
				continue;
			}
		}
...
int G_FindLocalInterestPoint( gentity_t *self )
{
	...
	for ( i = 0; i < level.numInterestPoints; i++ )
	{
		...
		if ( SV_inPVS( level.interestPoints[i].origin, eyes ) )
		{
