...
gentity_t	*G_TestEntityPosition( gentity_t *ent ) {
	...
	if ( ent->client ) {
		...
		SV_Trace( &tr, ent->client->ps.origin, ent->r.mins, vMax, ent->client->ps.origin, ent->s.number, mask, qfalse, 0, 0 );
	} else {
		SV_Trace( &tr, ent->s.pos.trBase, ent->r.mins, ent->r.maxs, ent->s.pos.trBase, ent->s.number, mask, qfalse, 0, 0 );
	}
...
qboolean	G_TryPushingEntity( gentity_t *check, gentity_t *pusher, vec3_t move, vec3_t amove ) {
	...
	*/
	...
	if (!block) {
		...
		SV_LinkEntity ((sharedEntity_t *)check);
		return qtrue;
	}
...
qboolean G_MoverPush( gentity_t *pusher, vec3_t move, vec3_t amove, gentity_t **obstacle ) {
	...
	*obstacle = NULL;
	...
	SV_LinkEntity( (sharedEntity_t *)pusher );
	...
	for ( e = 0 ; e < listedEntities ; e++ ) {
		...
		*obstacle = check;
		...
		for ( p=pushed_p-1 ; p>=pushed ; p-- ) {
			...
			SV_LinkEntity ((sharedEntity_t *)p->ent);
		}
		return qfalse;
	}
...
void G_MoverTeam( gentity_t *ent ) {
	...
	if (part) {
		...
		for ( part = ent ; part ; part = part->teamchain ) {
			...
			SV_LinkEntity( (sharedEntity_t *)part );
		}
...
static void Touch_DoorTriggerSpectator( gentity_t *ent, gentity_t *other, trace_t *trace ) {
	...
	SV_Trace(&tr, origin, pMins, pMaxs, origin, other->s.number, other->clipmask, qfalse, 0, 0);
	if (!tr.startsolid &&
		!tr.allsolid &&
		tr.fraction == 1.0f &&
...
void Think_SpawnNewDoorTrigger( gentity_t *ent )
{
	...
	SV_LinkEntity ((sharedEntity_t *)other);
...
void SpawnPlatTrigger( gentity_t *ent ) {
	...
	SV_LinkEntity ((sharedEntity_t *)trigger);
}
...
void Think_SetupTrainTargets( gentity_t *ent ) {
	...
	for ( path = ent->nextTrain ; path != start ; path = next ) {
		...
		if ( !path->target ) {
//			Com_Printff( "Train corner at %s without a target\n",
//				vtos(path->s.origin) );
			//end of path
			break;
		}
		...
		do {
			...
			if ( !next ) {
//				Com_Printff( "Train corner at %s without a target path_corner\n",
//					vtos(path->s.origin) );
				//end of path
				break;
			}
...
void funcBBrushDieGo (gentity_t *self)
{
	...
	SV_AdjustAreaPortalState( (sharedEntity_t *)self, qtrue );
...
{
	//Com_Printf("Mr. Glass says: PLZ NO IT HURTS\n");
	//Make "cracking" sound?
}
