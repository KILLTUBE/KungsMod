...
qhandle_t R_RegisterShaderLightMap( const char *name, const int *lightmapIndexes, const byte *styles );
...
static qboolean ParseStage( shaderStage_t *stage, const char **text )
{
	...
	while ( 1 )
	{
		...
		else if(!Q_stricmp(token, "stage"))
		{
			...
			if(token[0] == 0)
			{
				R_Printf(PRINT_WARNING, "WARNING: missing parameters for stage in shader '%s'\n", shader.name);
				continue;
			}
			...
			{
				R_Printf(PRINT_WARNING, "WARNING: unknown stage parameter '%s' in shader '%s'\n", token, shader.name);
				continue;
			}
		}
		//
	...
	qboolean useLightVector, qboolean useLightVertex, qboolean parallax, qboolean tcgen)
{
	...
	//R_Printf(PRINT_ALL, "shader %s has diffuse %s", shader.name, diffuse->bundle[0].image[0]->imgName);
	...
	if (r_normalMapping->integer)
	{
		...
		if (normal)
		{
			//R_Printf(PRINT_ALL, ", normalmap %s", normal->bundle[0].image[0]->imgName);
	...
	if (r_specularMapping->integer)
	{
		if (specular)
		{
			//R_Printf(PRINT_ALL, ", specularmap %s", specular->bundle[0].image[0]->imgName);
...
qhandle_t R_RegisterShaderFromImage(const char *name, const int *lightmapIndexes, const byte *styles, image_t *image, qboolean mipRawImage) {
	int			hash;
	shader_t	*sh;
