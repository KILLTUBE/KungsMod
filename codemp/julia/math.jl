...
Base.pointer(this::Vec3) = Ptr{Float32}(pointer_from_objref(this))
Base.convert(::Type{Vec3}, address::Ptr{Float32})::Vec3 = unsafe_load(Ptr{Vec3}(address))

function Base.normalize(this::Vec3)::Vec3
	scale = 1 / length(this)
	return this * scale
end
...
Base.length(this::Vec3) = sqrt(this.x*this.x + this.y*this.y + this.z*this.z)
