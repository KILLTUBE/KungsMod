...
CreateNewDamageBox( ent_ ) = ccall((:CreateNewDamageBox, lib), gentity_t, ( Ptr{gentity_t},  ), ent_ );
...
FirstFreeTagOwner() = ccall((:FirstFreeTagOwner, lib), tagOwner_t, (),  );
FirstFreeRefTag( tagOwner_ ) = ccall((:FirstFreeRefTag, lib), reference_tag_t, ( Ptr{tagOwner_t},  ), tagOwner_ );
TAG_Init() = ccall((:TAG_Init, lib), Void, (),  );
TAG_FindOwner( owner_ ) = ccall((:TAG_FindOwner, lib), tagOwner_t, ( Ptr{Char},  ), owner_ );
TAG_Find( owner_, name_ ) = ccall((:TAG_Find, lib), reference_tag_t, ( Ptr{Char}, Ptr{Char},  ), owner_, name_ );
TAG_Add( name_, owner_, origin_, angles_, radius_, flags_ ) = ccall((:TAG_Add, lib), reference_tag_t, ( Ptr{Char}, Ptr{Char}, vec3_t, vec3_t, Int32, Int32,  ), name_, owner_, origin_, angles_, radius_, flags_ );
...
G_ClientForShooter() = ccall((:G_ClientForShooter, lib), gclient_t, (),  );
