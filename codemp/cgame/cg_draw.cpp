...
//	int val[5];
	...
	if ( cg.predictedPlayerState.zoomMode == 2 )
	{
		...
		R_SetColor( colorTable[CT_BLACK]);
...
*/
		...
		if ( cg.predictedPlayerState.weaponstate == WEAPON_CHARGING_ALT )
		{
			...
			R_StretchPic(257, 435, 134*max, 34, 0, 0, max, 1, cgs.media.disruptorChargeShader);
		}
//		R_SetColor( colorTable[CT_WHITE] );
//		CG_DrawPic( 0, 0, 640, 480, cgs.media.disruptorMask );
...
void CG_DrawHealth( menuDef_t *menuHUD )
{
	...
	{
		// Print health amount
		R_SetColor( focusItem->window.foreColor );
...
void CG_DrawAmmo( centity_t	*cent,menuDef_t *menuHUD)
{
	...
	{ //just draw "infinite"
		...
		R_SetColor( colorTable[CT_YELLOW] );
		if (focusItem)
		{
...
void CG_DrawHUD(centity_t	*cent)
{
	...
	if (cg.predictedPlayerState.pm_type != PM_SPECTATOR)
	{
		...
		if (menuHUD)
		{
			...
			{
				focusItem = Menu_FindItemByName(menuHUD, "scoR_line");
				if (focusItem)
				{
...
//	addX = (float) smallIconSize * .75;
	...
	for (iconCnt=0;iconCnt<sideLeftIconCnt;i--)
	{
		...
		if (cgs.media.invenIcons[i])
		{
			...
			R_SetColor(colorTable[CT_ICON_BLUE]);
...
float CG_DrawVehicleShields( const menuDef_t	*menuHUD, const centity_t *veh )
{
	...
	if (item)
	{
		R_SetColor( item->window.foreColor );
		CG_DrawPic(
			item->window.rect.x,
			item->window.rect.y,
			item->window.rect.w,
...
void CG_DrawVehicleTurboRecharge( const menuDef_t	*menuHUD, const centity_t *veh )
{
	...
	if (item)
	{
		...
		{
			percent = 1.0f;
			R_SetColor( colorTable[CT_GREEN] );
		}
		else
		{
						...
						if ( cent->currentState.otherEntityNum == cg.predictedPlayerState.clientNum || cent->currentState.otherEntityNum == cg.predictedPlayerState.m_iVehicleNum )
						{
							if ( radarLockSoundDebounceTime < cg.time )
							{
								...
								if ( actualDist > RADAR_MISSILE_RANGE * 0.66f )
								{
									...
									arrowBaseScale = 3.0f;
									alarmSound = S_RegisterSound( "sound/vehicles/common/lockalarm1.wav" );
								}
								else if ( actualDist > RADAR_MISSILE_RANGE/3.0f )
								{
									radarLockSoundDebounceTime = cg.time + 500;
									arrowBaseScale = 6.0f;
									alarmSound = S_RegisterSound( "sound/vehicles/common/lockalarm2.wav" );
								}
								else
								{
									radarLockSoundDebounceTime = cg.time + 250;
									arrowBaseScale = 9.0f;
									alarmSound = S_RegisterSound( "sound/vehicles/common/lockalarm3.wav" );
								}
...
void CG_DrawLagometer( void ) {
	...
	for ( a = 0 ; a < aw ; a++ ) {
		...
		if ( v > 0 ) {
			if ( color != 1 ) {
				color = 1;
				R_SetColor( g_color_table[ColorIndex(COLOR_YELLOW)] );
			}
			if ( v > range ) {
				v = range;
			}
			R_StretchPic ( ax + aw - a, mid - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		} else if ( v < 0 ) {
			if ( color != 2 ) {
				color = 2;
				R_SetColor( g_color_table[ColorIndex(COLOR_BLUE)] );
			}
			v = -v;
			if ( v > range ) {
				v = range;
			}
			R_StretchPic( ax + aw - a, mid, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		}
	}
	...
	for ( a = 0 ; a < aw ; a++ ) {
		...
		if ( v > 0 ) {
			if ( lagometer.snapshotFlags[i] & SNAPFLAG_RATE_DELAYED ) {
				...
				if ( color != 3 ) {
					color = 3;
					R_SetColor( g_color_table[ColorIndex(COLOR_GREEN)] );
				}
			}
			v = v * vscale;
			if ( v > range ) {
			...
			R_StretchPic( ax + aw - a, ay + ah - v, 1, v, 0, 0, 0, 0, cgs.media.whiteShader );
		} else if ( v < 0 ) {
			if ( color != 4 ) {
				color = 4;		// RED for dropped snapshots
				R_SetColor( g_color_table[ColorIndex(COLOR_RED)] );
			}
			R_StretchPic( ax + aw - a, ay + ah - range, 1, range, 0, 0, 0, 0, cgs.media.whiteShader );
		}
	}
...
void CG_DrawCrosshair( vec3_t worldPoint, int chEntValid ) {
	...
	R_StretchPic( chX, chY, w, h, 0, 0, 1, 1, hShader );
	...
	if ( corona ) // drawing extra bits
	{
		...
		R_StretchPic( x + cg.refdef.x + 0.5 * (640 - w),
...
void CG_DrawSiegeHUDItem(void)
{
	...
	if (handle)
	{
		R_ModelBounds( handle, mins, maxs );
	}
	else
	{
...
void CG_DrawMiscStaticModels( void ) {
	...
	for( i = 0; i < cgs.numMiscStaticModels; i++ ) {
		...
		if( !R_inPVS( cg.refdef.vieworg, cullorg, cg.refdef.areamask ) ) {
			continue;
		}
