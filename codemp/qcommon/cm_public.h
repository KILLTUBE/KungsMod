...
CCALL void		CM_BoxTrace ( trace_t *results, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, int brushmask, int capsule );
CCALL void		CM_TransformedBoxTrace( trace_t *results, const vec3_t start, const vec3_t end, const vec3_t mins, const vec3_t maxs, clipHandle_t model, int brushmask, const vec3_t origin, const vec3_t angles, int capsule );
CCALL byte		*CM_ClusterPVS (int cluster);
CCALL int			CM_PointLeafnum( const vec3_t p );
// only returns non-solid leafs
// overflow if return listsize and if *lastLeaf != list[listsize-1]
CCALL int			CM_BoxLeafnums( const vec3_t mins, const vec3_t maxs, int *boxList,
...
//rwwRMG - added:
CCALL bool		CM_GenericBoxCollide(const vec3pair_t abounds, const vec3pair_t bbounds);
CCALL void		CM_CalcExtents(const vec3_t start, const vec3_t end, const struct traceWork_s *tw, vec3pair_t bounds);

// cm_tag.c
CCALL int			CM_LerpTag( orientation_t *tag,  clipHandle_t model, int startFrame, int endFrame, float frac, const char *tagName );


// cm_marks.c
CCALL int	CM_MarkFragments( int numPoints, const vec3_t *points, const vec3_t projection,
...
// cm_trace.cpp
CCALL bool CM_CullWorldBox (const cplane_t *frustum, const vec3pair_t bounds);
